...
на русском и английском, журнал аудита и игровая админ-панель (F9) на Dear
ImGui игры, которую сервер сам отдаёт каждому игроку — игроку ничего
ставить не нужно. Все решения принимает сервер; панель только показывает и
просит.
...
3. Для панели больше ничего не нужно: `resources/warden/client/warden/*.lua`
   лежит в том же архиве, сервер отдаёт эти файлы каждому входящему игроку.
   **F9** открывает и закрывает её (`ui.key` в конфиге задаёт другую
   клавишу); `/wd` в чате делает то же для игрока, у кого F9 занята.
...
**F9** (или `/wd`) открывает окно `Warden`; что в нём видно, зависит от прав
игрока, а кнопка, которую отклонило бы правило уровней, серая — сервер всё
равно проверяет каждый запрос, и его отказ печатается на месте кнопки.

| Вкладка | Нужно | Что показывает |
|---|---|---|
| Игроки | `players.view` | Все в сети: id, аккаунт/гость, группа, машины, пинг, время в сети; запись выбранного игрока (ключ, IP, входы, предупреждения, мьют, лимит) и действия: кик, предупреждение, мьют, снять мьют, временный бан, бан, группа, удалить машины, в белый список, голосование за кик — с формой для причины, срока (30m / 2h / 1d / 7d / другой) и группы |
| Группы | — | Группы с уровнем, наследованием, лимитом и правами (только просмотр; группа игрока задаётся во вкладке «Игроки») |
| Настройки | `settings.read` (+ `settings.write` для правки) | Настройки на лету с полями по типу (флажок, число, выбор) и «Сбросить», где значение отличается от `resource.toml` |
| Аудит | `audit.view` | Последние N записей журнала |
| Баны | `mod.ban` | Баны с кнопкой «Разбанить» |
...
| `ui.key` | string | `"F9"` |  | Клавиша панели: имя клавиши Dear ImGui (`F9`, `F7`, `Insert`); `/wd` в чате тоже переключает её. |
<!-- config-doc:end -->
