...
tests.identity_keys_and_history = function()
    ...
    -- find: pid, key, online name, offline name; a prefix only for the read-only kinds (fuzzy), online
    t.eq(W.identity.find("#1"), "acct:42")
    t.eq(W.identity.find("acct:42"), "acct:42")
    t.eq(W.identity.find("alicia"), "acct:42")
    t.eq(W.identity.find("ali", { fuzzy = true }), "acct:42", "unique prefix of a connected name, fuzzy only")
    t.eq(W.identity.find("ali"), nil, "no prefix pick for the mutating kinds")
    node._leave(1)
    t.eq(W.identity.find("Alicia"), "acct:42", "offline, from the history")
    t.eq(W.identity.find("ali", { fuzzy = true }), nil, "a prefix never reaches into the history")
